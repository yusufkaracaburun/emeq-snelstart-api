...
# `actieprijzen` — API reference

_SnelStart B2B-Api v2 v1.0_ — base URL: `https://b2bapi.snelstart.nl/v2`

> **AI context:** use this file as the authoritative reference when generating SnelStart B2B-API integration code for the `actieprijzen` resource. Each operation below lists its HTTP method + path, required OAuth scopes (if any), request body schema, and response schemas (with links into `_schemas.md`).
...
_/v2/actieprijzen (GET) (OData)_

**operationId:** `v2-actieprijzen-GET-OData`
**Required scopes:** `settings:read`

Geeft alle beschikbare actieprijzen (ActieprijzenModel) terug uit een administratie. 
...
- **200** → [`SnelStart-B2B-Api-V2-Models-Actieprijzen-ActieprijzenModelArray`](_schemas.md#snelstart-b2b-api-v2-models-actieprijzen-actieprijzenmodelarray) (`application/json`) — HTTP 200 OK
